...
IniFile::Section::Section(const std::string& name) : name_(name) {}
...
void IniFile::Section::Set(const char* key, double newValue) {
	Set(key, StringFrom(newValue).c_str());
}

void IniFile::Section::Set(const char* key, signed newValue) {
	Set(key, StringFrom(newValue).c_str());
}

void IniFile::Section::Set(const char* key, unsigned newValue) {
	Set(key, StringFrom(newValue).c_str());
}

void IniFile::Section::Set(const char* key, bool newValue) {
	Set(key, StringFrom(newValue).c_str());
}
...
bool IniFile::Section::Get(const char* key, signed* value, signed defaultValue)
{
	std::string temp;
	bool retval = Get(key, &temp, 0);
	if (retval && TryParse(temp.c_str(), value))
...
bool IniFile::Section::Get(const char* key, unsigned* value, unsigned defaultValue)
{
	std::string temp;
	bool retval = Get(key, &temp, 0);
	if (retval && TryParse(temp.c_str(), value))
